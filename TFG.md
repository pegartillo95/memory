...
En un primer lugar esta clase estaba pensada para utilizar la librería Generics para ser una única clase y para contar con un método **compose** con la cual ser capaces de generar instancias de la clase **Allv** para tipos definidos por el usuario. Dicha función se encargaría de crear la lista devuelta mediante **allv** para el nuevo tipo de datos a partir de las listas **allv** de los tipos predefinidos, pero encontramos un problema a la hora de integrarlo con la clase Sized y la idea que teniamos con darle al usuario la posibilidad de pedir los n valores mas pequeños de una clase o los n primeros valores de tamaño menor o igual a un número prefijado por el. El caso es que se descartó utilizar la librería **Generics** debido a un problema que surgía cuando alguna de las listas se trataba de una lista infinita.
...
Adjunto aqui el codigo de las 4.
